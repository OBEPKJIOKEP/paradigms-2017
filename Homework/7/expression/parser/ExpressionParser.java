...
public class ExpressionParser implements Parser {
    ...
    public TripleExpression parse(String expression) throws ParsingException {
        ...
        if (tokens.get(current).getType() != TokenType.EOL) {
            ...
            if (extraToken.getType() == TokenType.NUMBER ||
                    extraToken.getType() == TokenType.VARIABLE) {
                throw new MissingOperationException(extraToken);
            } else {
    ...
    private TripleExpression parseAddSub() throws ParsingException{
    ...
    private TripleExpression parseMulDiv() throws ParsingException{
        TripleExpression left = parseUnary();
        while (test("*") || test("/")) {
            if (test(current - 1, "*")) {
    ...
    private TripleExpression parseConstOrVariable() throws ParsingException {
        ...
        if (test(TokenType.NUMBER)) {
            ...
            try {
                ...
                throw new ConstantAssignmentException(firstPosition, number);
            }
        }
